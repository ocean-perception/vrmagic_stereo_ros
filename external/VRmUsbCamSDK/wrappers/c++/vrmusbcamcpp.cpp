...
#define is_not !=	//
	...
	Image::Image(const ImageFormat& format, void* buffer, int pitch, int buffer_size)
		: mp_vrmimage(0), mp_owner(ImagePtr())
	{
		VRMCHECK(buffer is_not null);
		VRMCHECK(pitch>=0);
		VRMCALL(SetImageEx(&mp_vrmimage, format.get_NativeRepresentation(), (VRmBYTE*)buffer, pitch, buffer_size));
	}
	...
	Image::~Image() {
		if (mp_vrmimage) {
			DevicePtr* lpp_device= boost::get<DevicePtr>(&mp_owner);
			if ( lpp_device && *lpp_device )
...
#define RETURNGETVALUE(vrm_type, vrm_suffix) \
	...
	VRmUsbCamCPP::EventReturnType Device::WaitForStrobe( int f_timeout_us )
	{
		VRMCHECK(m_device is_not null);
		VRmUsbCamCPP::EventReturnType tmp;
		VRMDEVCALL(WaitForStrobe(m_device, f_timeout_us, &tmp));
		return tmp;
	}

	void Device::SetUserLut( VRmDWORD f_port, VRmDWORD f_channel, UserLutFormat f_format, const std::vector<VRmWORD> &fcr_lut )
	{
		VRMCHECK(m_device is_not null);
		VRMCHECK(fcr_lut.size() > 0);

		VRMDEVCALL(SetUserLut(m_device, f_port, f_channel, f_format, &fcr_lut[0]));
	}

	std::vector<VRmWORD> Device::GetUserLut( VRmDWORD f_port, VRmDWORD f_channel, UserLutFormat f_format )
	{
		VRMCHECK(m_device is_not null);

		VRmWORD *lp_user_lut_tmp = 0;
		VRMDEVCALL(GetUserLut(m_device, f_port, f_channel, f_format, &lp_user_lut_tmp));

		size_t l_user_lut_size = 0;
		switch (f_format)
		{
		case VRM_USER_LUT_8_TO_16:
			l_user_lut_size = 1 << 8;
			break;
		case VRM_USER_LUT_10_TO_16:
			l_user_lut_size = 1 << 10;
			break;
		case VRM_USER_LUT_16_TO_16:
			l_user_lut_size = 1 << 16;
			break;
		default:
			VRMERR("UserLutFormat is not valid");
		}

		std::vector<VRmWORD> ret;
		// copy lp_user_lut_tmp's contents into ret
		ret.insert(ret.end(), &lp_user_lut_tmp[0], &lp_user_lut_tmp[l_user_lut_size]);

		VRMCALL(FreeUserLut(&lp_user_lut_tmp));

		return ret;
	}

	void Device::UserLutCommit( VRmDWORD f_port )
	{
		VRMCHECK(m_device is_not null);
		VRMDEVCALL(UserLutCommit(m_device, f_port));
	}
	...
	void VRmUsbCam::UpdateDeviceKeyList(bool local /* = false */, bool usb /* = false */, bool ethernet /* = false */) {
		if (local || usb || ethernet)
		{
			VRMCALL(UpdateDeviceKeyListEx(local, usb, ethernet));
		}
		else
		{
	...
	void VRmUsbCam::ResetDeviceKeyList()
	{
		VRMCALL(ResetDeviceKeyList());
	}
	...
	ImagePtr VRmUsbCam::SetImage(const ImageFormat& format, void* buffer, int pitch, int buffer_size) {
		return ImagePtr(new Image(format, buffer, pitch, buffer_size));
	}
