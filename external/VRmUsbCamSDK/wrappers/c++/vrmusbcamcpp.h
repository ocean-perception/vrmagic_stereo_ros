// ==============================================================================================
/// @file vrmusbcamcpp.h                                           VRmUsbCam C++ Wrapper v3.5.0.0
//  Copyright VRmagic 2014
//
//  vrmusbcamcpp.cpp
// ==============================================================================================
...
namespace VRmUsbCamCPP {
	...
	/// enum for event return value
	typedef VRmEventReturnType EventReturnType;
	/// enum for event return value
	typedef VRmUserLutFormat UserLutFormat;
	...
	private:
		...
		// constructor used by VRmUsbCam::SetImage()
		Image(const ImageFormat& format, void* buffer, int pitch, int buffer_size);
	...
	public:
		...
		/// blocks until strobe interrupt signal was emitted by kernel (D3 camera only)
		EventReturnType WaitForStrobe(int f_timeout_us);

		/// user LUT access

		/// set user LUT on the device
		/// this LUT is applied in addition to the correction LUT
		/// parameters 
		///     f_port: affected sensor port, use 1 for single sensor cameras
		///     f_channel: affected lut channel 0 for monochrome, 0-3 for Bayer
		///     f_format: choose correct LUT format, matching the LUT buffer and device settings
		///     fcr_lut: LUT buffer, length and content have to match the format
		void SetUserLut(VRmDWORD f_port, VRmDWORD f_channel, UserLutFormat f_format, const std::vector<VRmWORD> &fcr_lut);

		/// get the LUT that is associated with a specific sensor port.
		/// parameters 
		///     f_port: affected sensor port, use 1 for single sensor cameras
		///     f_channel: affected lut channel 0 for monochrome, 0-3 for Bayer
		///     f_format: choose required LUT format, internal format is converted to requested format
		///     return: requested LUT buffer
		std::vector<VRmWORD> GetUserLut(VRmDWORD f_port, VRmDWORD f_channel, UserLutFormat f_format);

		/// commit user LUT to the device
		/// call this function after setting the user lut to write the lut to the device
		/// parameters 
		///     f_port: affected sensor port, use 1 for single sensor cameras
		void UserLutCommit(VRmDWORD f_port);
	...
	public:
		...
		/// use parameters to control which devices are listed.
		/// f_local: list local devices (on intelligent camera)
		/// f_usb: list devices connected via USB
		/// f_ethernet: list devices connected via Ethernet
		/// if all 3 parameters are false, it uses the configuration specified by vrmgrbdevice.ini
		static void UpdateDeviceKeyList(bool local = false, bool usb = false, bool ethernet = false);
		/// trigger a full rescan of the device key list on the the call to UpdateDeviceKeyList
		/// usage: use this function immediately followed by a call to UpdateDeviceKeyList
		static void ResetDeviceKeyList();
		...
		/// create a new Image as container for a given format, buffer, pitch and buffer size
		/// the buffer must remain valid for the lifetime of the created object and is not automatically deallocated when the Image object is released.
		static ImagePtr SetImage(const ImageFormat& format, void* buffer, int pitch, int buffer_size);
